...
export class Lending extends IdEntity {
  ...
  @Fields.string({
    ...
    validate: (_, f) => {
      ...
      if (f.value.startsWith('972')) f.value = '0' + f.value.substr(3);
      else if (orig.trim().startsWith('+')) f.value = '+' + f.value;
      if (f.value.length < 10) throw Error('טלפון לא תקין');
    },
  })
  phone = '';
  ...
  @Field(() => Item, {
    allowApiUpdate: Roles.admin,
    displayValue: (_, l) => l.name,
  })
  @DataControl({
    readonly: true,
    valueList: async (r: Remult) => getEntityValueList(r.repo(Item)),
  })
  item!: Item;
  ...
  sendPostMessage() {
    let message = `שלום ${this.firstName}, 
תודה שהשתמשתם במערך הסיוע של היופי שבנתינה להשאלת ציוד רפואי.
שמחנו לסייע ואנו מקווים שאכן הציוד עזר לכם.

נודה לכם אם תוכלו לקחת שותפות בהמשך הפעילות ובפיתוח של היופי שבנתינה, בקישור הבא: https://bit.ly/netina-tzoran
...
${window.location.origin + '/form/' + this.id}`;
  ...
  sendWhatsapp(message: string) {
    ...
    if (phone.startsWith('0')) phone = '972' + phone.substr(1);
